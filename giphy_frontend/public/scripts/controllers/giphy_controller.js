function GiphyController($scope, $http, $state, $stateParams, $rootScope) {
  ...
  function populateInitialState(user) {
    $http.get(`${server}/users/${user._id}/gifs`)
  ...
  function getSavedGifs(currentUser) {
    $http.get(`${server}/users/${currentUser._id}/gifs`)
      .then(function(response) {
        self.savedGifs = response.data.gifs
  ...
  function saveGif(url, currentUser) {
    ...
    $http.post(`${server}/users/${currentUser._id}/gifs`, { url: url, name: self.name } )
  ...
  function updateGif(currentUser) {
    $http.put(`${server}/users/${currentUser._id}/gifs/${$stateParams.gifId}`, { name: self.name, url: self.url } )
      .then(function(giphyResponse) {
  ...
  function deleteGif(id, currentUser) {
    console.log(id)
    $http.delete(`${server}/users/${currentUser._id}/gifs/${id}`)
      .then(function(response) {
